        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 my-2.5 min-h-0 items-start lg:items-stretch">
          ...
          <section className={`${roomLayout === 'focus' ? 'col-span-12' : 'col-span-12 lg:col-span-8 xl:col-span-8'} flex flex-col gap-2.5 min-w-0`}>
            ...
            {isMounted && !dismissedHints.has(STORAGE_KEYS.HINT_VOLUME) && (
              <div className="flex items-center justify-between gap-2 px-2 text-[11px] text-gray-500 font-medium">
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-gray-700 transition ml-2 shrink-0 cursor-pointer"
                >✕</button>
              </div>
            )}
            <DualVolumeMixer
          ...
          {roomLayout !== 'focus' && (
            <>
              ...
              <aside
                className={`${
                  isMobileDrawerOpen
                    ? 'fixed inset-x-0 bottom-0 z-50 h-[75vh] max-h-[75vh] sm:h-[80vh] rounded-t-3xl border-t border-black/10 bg-white p-3.5 flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300'
                    : 'hidden md:flex'
                } md:col-span-12 lg:col-span-4 xl:col-span-4 flex-col gap-2.5 min-w-0 lg:h-full lg:max-h-[calc(100vh-140px)]`}
              >
                {/* Mobile Drawer Header: Drag Handle & Close Button */}
                <div className="flex md:hidden items-center justify-between px-1 pb-1.5 border-b border-black/10 shrink-0">
                  <div className="w-10 h-1 rounded-full bg-black/20 mx-auto" />
                  <button
                    type="button"
                    onClick={() => setIsMobileDrawerOpen(false)}
                    className="p-1 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-black/5 transition cursor-pointer"
                    aria-label="Close drawer"
                  >
                    <X className="w-4 h-4" />
                  </button>
                ...
                {isMounted && !dismissedHints.has(STORAGE_KEYS.HINT_TRIVIA) && (
                  <div className="shrink-0 flex items-center justify-between px-3 py-2 rounded-xl bg-orange-50/90 border border-orange-200/90 text-xs text-gray-800 shadow-2xs">
                    <div className="flex items-center gap-2 truncate">
                      <span className="text-sm">🎮</span>
                      <span className="truncate">Switch to <strong className="text-[#EA580C] font-bold">Trivia</strong> mode in the header during breaks.</span>
                    </div>
                    <button
                      onClick={() => dismissHint(STORAGE_KEYS.HINT_TRIVIA)}
                      aria-label="Dismiss trivia tip"
                      className="text-gray-400 hover:text-gray-700 transition ml-2 shrink-0 cursor-pointer p-1"
                    >✕</button>
                  </div>
                )}

                {/* Tab Switcher Header (Desktop, Tablet & Mobile Drawer) */}
                <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-gray-100/90 border border-black/5 shadow-2xs shrink-0">
                  {/* Tab 1: Video Call */}
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span>Call</span>
                    <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-orange-100 font-mono text-[#EA580C] font-bold">
                  ...
                  <button
                    ...
                    className={`flex-1 relative flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl text-xs font-bold transition cursor-pointer ${
                  ...
                  >
                    ...
                    {unreadChatCount > 0 && activeSidebarTab !== 'chat' && (
                      <span className="px-1.5 py-0.2 rounded-full bg-[#FF5722] text-white font-mono text-[10px] font-bold shadow-2xs animate-pulse">
                  ...
                  >
                    <BarChart3 className="w-3.5 h-3.5 text-[#FF5722]" />
                    <span>Poll</span>
              ...
              <div className="fixed bottom-0 inset-x-0 z-30 p-2.5 bg-white/95 backdrop-blur-md border-t border-black/10 md:hidden flex items-center justify-between gap-2 shadow-2xl">
                <button
                  type="button"
                  onClick={() => handleOpenMobileDrawer('call')}
                  className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl text-xs font-bold border transition cursor-pointer ${
                    activeSidebarTab === 'call' && isMobileDrawerOpen
                      ? 'bg-orange-50 text-[#EA580C] border-orange-200 shadow-2xs'
                      : 'bg-gray-50 text-gray-700 border-black/5 hover:bg-gray-100'
                  }`}
                >
                  <Users className="w-3.5 h-3.5 text-[#FF5722]" />
                ...
                <button
                  ...
                  className={`flex-1 relative flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl text-xs font-bold border transition cursor-pointer ${
                ...
                <button
                  ...
                  className="p-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 transition shrink-0 cursor-pointer border border-black/5"
                >
